...
export class FilterIterator<T> extends IteratorClass<T> {
  ...
  constructor(it: { [Symbol.iterator](): Iterator<T> }, protected fn: (value: T, index: number) => boolean) {
    super()
    this.it = it[$$iterator]()
  }
...
export class FilterIterable<T> extends IterableClass<T> {
  constructor(source: { [Symbol.iterator](): Iterator<T> }, fn: (value: T, index: number) => boolean) {
